...
class System(models.Model):
    ...
    def __unicode__(self):
        return ('No: ' + str(self.id)
                ...
                + ' from family ' + self.family_composition.replace('_','')
                + ' subunit_family_composition ' + self.subunit_family_composition
...
class ProteinForm(forms.ModelForm):
    ...
    dnapiece = forms.ModelChoiceField(queryset=DNAPiece.objects.all())
    genome = forms.ModelChoiceField(queryset=Genome.objects.all())
    subunit_kind = forms.CharField(max_length=5, widget=forms.Select(choices=SUBUNIT_KINDS))
    class Meta:
        model = Protein
        fields = ('genome', 'dnapiece', 'system', 'gene_id', 'margin_left', 'system_part', 'hammer_cluster', 'subunit_kind', 
                  'aa_sequence', 'hh_pfam_id', 'hh_pfam_desc')
